...
int test_connect_with_will() {
    ...
    PubSubClient client(server, 1883, callback, shimClient);
    int rc = client.connect((char*)"client_test1",(char*)"willTopic",1,0,(char*)"willTheMessage");
    IS_TRUE(rc);
    IS_FALSE(shimClient.error());
...
int test_connect_with_will_username_password() {
    ...
    PubSubClient client(server, 1883, callback, shimClient);
    int rc = client.connect((char*)"client_test1",(char*)"user",(char*)"password",(char*)"willTopic",1,0,(char*)"willTheMessage");
    IS_TRUE(rc);
    IS_FALSE(shimClient.error());
